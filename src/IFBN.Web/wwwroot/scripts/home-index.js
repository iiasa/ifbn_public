...
(function (scope) {
	...
	var styleFunction_polygons = function (feature) {
	...
	var styleFunction_clustered = function (feature) {
	...
	scope.map = (function () {
		...
		self.initialize = function (plotLocations, confidentialPlotLocations) {
			...
			var map = new ol.Map({
				...
				layers: [
						...
						layers: [
							...
							new ol.layer.Tile({
								...
								source: new ol.source.TileWMS(({
									...
									serverType: "mapserver",
									attributions: [
										new ol.Attribution({ html: '<a href="https://doi.org/10.1109/JSTARS.2018.2851606" target="_blank" style="font-size: 12px; display: block; text-align: left; font-weight: bolder">LiDAR map reference</a><br/>' })
									]
								}))
							}),
							new ol.layer.Tile({
					...
					new ol.layer.Group({
						title: "clusters",
						layers: [
							clusters_plotFeature, clusters_confidentialPlotFeatures
						]
					})
				],
				view: new ol.View({
			...
			var displayFeatureInfo = function (pixel, data) {
				info.css({
					left: (pixel[0] + 15) + "px",
					top: (pixel[1] /* - 5 */) + "px"
				});
							...
							function (data) {
								if (!IsNullOrUndefined(data)) {
									...
									if (!IsNullOrUndefined(data.censuses) && data.censuses.length > 0) {
										...
										if (!IsNullOrUndefined(census.measurements) && census.measurements.length > 0) {
											...
											for (var i = 0; i < census.measurements.length; i++) {
												...
												if (census.measurements[i].additionalMeasurements !== null &&
													census.measurements[i].additionalMeasurements.length > 0) {
										...
										if (!IsNullOrUndefined(census.taxonomicIdentifications) &&
											census.taxonomicIdentifications.length > 0) {
									...
									if (data.photos.length > 0) {
										self.viewModel
											.photoUrl("https://forest-observation-system.net/Images/sites/" +
												data.photos[0]);
									}
